// test-collector.ts - Test script for RSS collector
import { config } from 'dotenv';
config();

import { RSSCollector } from '../collectors';
...
async function testMayorNewsroom() {
  ...
  try {
    ...
    const signals = await collector.getPool().query(query);
    ...
    const categories = await collector.getPool().query(categoryQuery);
